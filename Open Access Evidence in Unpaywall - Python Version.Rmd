...
To contrast the previous results from the blog article with the recent results, we begin with a comparison of the total number of articles between the two datasets from February 2019 and November 2019. More importantly, we investigate the open access share between these two.

Before we can compute the open access proportion in our dataset, we query our database and count the number of distinct DOI´s by year and open access status. Since we have two tables, we need to concatenate our dataframes in the next step. Also we have to convert the column which contains the year from string to datetime. This allows us to handle date informations much better when visualizing data. After we calculate the proportion of open access by year, we sort the values by time.
...
Like in the aforementioned blog article, we create a `host` column with the pandas provided `loc` method to highlight freely available full-texts provided by DOAJ in addition to the regular host types. Because DOAJ (Directory of Open Access) ensures high quality standards and peer reviews for their indexed journals, it might be interesting to see, wether an increase of publishing in potentially less strict open access journals can be observe. 
...
Table: Table 1: Number of articles per evidence type. 
...
It is interessting to see, that the evidence type with the highest number of related articles was not included in the previous database snapshot.
...
While openly forms of open access provision identified via pdf or license are still evidence types with the most often unique appearance, it can be seen that the share has decreased compared to the results from the previous database snapshot. On the other hand, the share of unique occurences of less frequent evidence types which were collated in the category other has increased rapidly.
