...
In this blog post, we discuss the results from the blog article "[Open Access Evidence in Unpaywall](https://subugoe.github.io/scholcomm_analytics/posts/unpaywall_evidence/)" by Najko Jahn and Anne Hobert by comparing different data dumps obtained from [Unpaywall](https://unpaywall.org/). Since the article was published in May 7 2019, Unpaywall has released a new data dump with up-to-date informations about scholarly journal articles. So it is interesting to see, how the results of the previous blog article are impacted. For this post, we are using the Unpaywall data dump from November 2019.

Although this work serves as a complete translation of the source code from R into Python, the main purpose was to update the results from the blog article.
...
In the following, we explore varietes between the evidence types of the two data dumps. The evidence type is a variable determined by Unpaywall, which expresses the location, in that an article was found and how Unpaywall was able to identify, that the status of an article is open access.
